...
always_ff @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        counter <= counter_nxt;
        state <= state_nxt;
        coming_note <= music_note;
        current_note <= (state == PLAYING) ? current_note_nxt : music_note;
    end
end

always_comb begin: state_blk
    case(state)
        IDLE: state_nxt = (enable) ? PLAYING : IDLE;
...
always_comb begin: counter_blk
    ...
    case(state)
        ...
        PLAYING: begin
            if(tick_in) begin
                if(counter <= 0) counter_nxt = coming_note.duration;
...
always_comb begin: notes_out_blk
    ...
    case(state)
        ...
        PLAYING: begin
            if(counter <= 0) notes_nxt = coming_note.note_pitch;
            else notes_nxt = current_note.note_pitch;
        end
    endcase
...
always_comb begin: note_addr_blk
    ...
    case(state)
        ...
        PLAYING: if(counter == 1) note_addr_nxt = note_addr + 1;
    endcase
